...
TEST_P(PairingTest, Run) {
  ...
  // phase 4
  // NOTE: clientpairingsecret() previously took an `add_cert` queue and pushed the
  // verified client certificate to it. Cert storage was later refactored to store the
  // cert internally (nvhttp's client_root.named_devices, which is not exposed for
  // inspection here), so the queue parameter was removed. root.paired now gates
  // phase-4 success (it is only set after the cert is verified and stored).
